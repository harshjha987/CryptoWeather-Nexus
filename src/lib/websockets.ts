...
export function initializeWebSocket() {
  ...
  const setupWebSocket = () => {
    try {
      ...
      ws = new WebSocket(
        `wss://ws.coincap.io/prices?assets=bitcoin,ethereum,ripple,cardano,solana,polkadot,dogecoin,litecoin,chainlink&apiKey=${CRYPTO_API_KEY}`,
      )
      ...
      ws.onopen = () => {
        ...
        // Clear any simulation interval if it's running
...
function handleCryptoUpdate(data: Record<string, string>) {
  try {
    ...
    console.error("Error handling crypto update:", error)
  }
}
...
function simulateWebSocketMessage() {
  try {
    ...
    console.error("Error in simulateWebSocketMessage:", error)
  }
}
...
function simulateWeatherAlert() {
  try {
    ...
    const alertTypes = [
      ...
      {
        type: "Heat Wave",
        message: `Heat wave warning for ${randomCity}. Temperatures expected to rise significantly.`,
      },
    ...
    console.error("Error in simulateWeatherAlert:", error)
  }
}
